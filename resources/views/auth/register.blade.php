<x-guest-layout>
  ...
  <div class="space-y-4 p-6 md:space-y-6">
    ...
    <form class="space-y-4" action="{{ route('register') }}" method="post">
      ...
      <div>
        <label class="mb-2 block text-sm font-medium text-zinc-900" for="password_confirmation">Konfirmasi Kata Sandi</label>
        <div class="relative">
          <input class="block w-full rounded-lg border border-zinc-300 bg-zinc-50 p-2.5 text-zinc-900 focus:border-blue-600 focus:ring-blue-600 sm:text-sm" id="password_confirmation" name="password_confirmation" type="password" placeholder="••••••••" required>
          <button class="absolute inset-y-0 right-2 top-1/2 -translate-y-1/2 transform cursor-pointer text-zinc-600" id="togglePassword" name="togglePassword" type="button">
            <svg class="h-5 w-5" id="x-lucide-eye" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0" />
              <circle cx="12" cy="12" r="3" />
      ...
      <p class="text-sm font-light text-zinc-500 dark:text-zinc-400">
        Sudah Memiliki Akun? <a class="font-medium text-blue-600 hover:underline dark:text-blue-500" href="{{ route('login') }}">Masuk</a>
      </p>
    </form>
  </div>
